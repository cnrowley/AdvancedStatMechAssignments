...
def train_and_evaluate(X_train, X_val, X_test, y_train, y_val, y_test, feature_names):
    ...
    model = GridSearchCV(RandomForestClassifier(random_state=random_seed, n_jobs=num_cpus), param_grid, cv=3, scoring='accuracy')
    ...
    for dataset, X, y in [('Train', X_train, y_train), ('Validation', X_val, y_val), ('Test', X_test, y_test)]:
        ...
        results.append(['Random Forest', dataset, accuracy_score(y, y_pred), report['1']['precision'], report['1']['recall'], report['1']['f1-score'], training_time])
    ...
    if hasattr(best_model, 'feature_importances_'):
        ...
        importance_series = pd.Series(best_model.feature_importances_, index=feature_names)
        sorted_importance_series = importance_series.sort_values(ascending=False)  # Sort by importance
    
        # Print feature importances in a clean format
        for feature, importance in sorted_importance_series.items():
...
def evaluate_secondary_test(model, scaler, test_file, feature_names):
    ...
    results = [ ['Random Forest', 'Secondary Test', accuracy_score(y_true, y_pred), report['1']['precision'], report['1']['recall'], report['1']['f1-score'] ]]
